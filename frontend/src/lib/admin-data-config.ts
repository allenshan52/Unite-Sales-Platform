...
const channelPartnerFields: AdminFieldConfig[] = [
  ...
  { name: "display_longitude", label: "演示经度", kind: "number", required: true, step: "any", min: 72.004, max: 137.8347, defaultValue: "104.1" },
  { name: "display_latitude", label: "演示纬度", kind: "number", required: true, step: "any", min: 0.8293, max: 55.8271, defaultValue: "35.6" },
    ...
    resources: [
      ...
      },
      {
        ...
        fields: [
          ...
          { name: "deal_type", label: "成交类型", kind: "text", required: true, maxLength: 80 },
      ...
      {
        key: "competitor_strength_regions",
        label: "强势区域",
        singular: "同行强势区域",
        description: "维护人工确认的省市级强势区域和判断依据。",
        listFields: ["province", "competitor_id", "region_level", "strength_level"],
        fields: [
          { name: "competitor_id", label: "所属同行", kind: "foreign", required: true, foreignResource: "competitors" },
          { name: "region_level", label: "区域级别", kind: "select", required: true, options: ["省", "市"] },
          { name: "province", label: "省份", kind: "text", required: true, maxLength: 60 },
          { name: "city", label: "城市", kind: "text", nullable: true, maxLength: 60 },
          { name: "strength_level", label: "强度", kind: "select", required: true, options: ["强", "中", "弱"] },
          { name: "source_type", label: "来源类型", kind: "select", required: true, options: sourceTypes },
          { name: "source_reference", label: "来源说明", kind: "text", required: true, wide: true, maxLength: 500 },
          { name: "source_url", label: "来源网址", kind: "text", nullable: true, wide: true, maxLength: 1000 },
          { name: "confidence", label: "置信度", kind: "select", required: true, options: confidenceLevels },
          { name: "basis", label: "判断依据", kind: "textarea", required: true, wide: true, maxLength: 5000 },
        ],
      },
      {
